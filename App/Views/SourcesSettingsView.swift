...
struct SourcesSettingsView: View {
    ...
    private let defaultAppTestersURL = "https://repository.apptesters.org/apps.json"
    private let oldAppTestersURL = "https://repository.apptesters.org"
    ...
    private func ensureCorrectDefaultSource() {
        var needsRefresh = false
        
        // إزالة الرابط القديم الخاطئ إذا كان موجوداً
        if let oldURL = URL(string: oldAppTestersURL), sources.sources.contains(oldURL) {
            sources.removeSource(oldURL)
            needsRefresh = true
        }

        // إضافة الرابط المباشر الصحيح تلقائياً إذا لم يكن موجوداً
        if let newURL = URL(string: defaultAppTestersURL), !sources.sources.contains(newURL) {
            sources.addSource(defaultAppTestersURL)
            needsRefresh = true
        }
