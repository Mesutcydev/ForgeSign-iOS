...
- **Install on device** — a local loopback HTTPS server serves the signed IPA via `itms-services` OTA install, with a silent-audio keep-alive so large IPAs survive backgrounding. A frosted-glass install page in Safari is the fallback on iOS versions that gate the direct handoff.
...
3. Install uses a self-hosted HTTPS endpoint (`*.backloop.dev` loopback identity) that serves `manifest.plist` + the IPA, then hands off to the iOS installer via `itms-services://`.
